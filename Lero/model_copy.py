...
class LeroModelPairWise(LeroModel):
    ...
    def fit(self, X1, X2, Y1, Y2, pre_training=False):
        ...
        train_dataset = DataLoader(pairs[:int(count*0.9)],
                             batch_size=batch_size,
                             shuffle=True,
                             collate_fn=collate_pairwise_fn)
        
        val_dataset = DataLoader(pairs[int(count*0.9):],
                                 batch_size=batch_size,
                                 shuffle=False,
                                 collate_fn=collate_pairwise_fn)

        optimizer = None
        optimizer = torch.optim.Adam(self._net.parameters(), weight_decay=0.001)
        ...
        for epoch in range(100):
            self._net.train() 
            loss_accum = 0
            for x1, x2, label in train_dataset:
            ...
            with torch.no_grad():
                val_loss = 0
                for x1, x2, label in val_dataset:
            ...
            print("Epoch", epoch, "training loss:", loss_accum, "validation loss:", val_loss)
            
            with open(str(int(start_time))+'_loss.txt','a') as f:
                f.write(f"Epoch: {epoch}, training loss: {loss_accum}, validation loss: {val_loss}\n")
